...
export async function GET(request: NextRequest) {
   ...
   try {
      ...
      if (make && !model) {
         ...
         query =
            "SELECT ID, Year, Transmission, Engine_displacement,Drive,Engine_descriptor,Fuel_type,Vehicle_size_class FROM VehicleData WHERE Make = ? AND Model = ? ORDER BY year DESC ";
         params = [make, model];
      } else {
